...
<%
    ...
    query_di_interrogazione = "SELECT COUNT(*) As Count FROM Student WHERE IsActive=0 AND Activation = '"& activation_code &"'"
    recordset.Open query_di_interrogazione,connessione

    If recordset("Count") = 1 Then
        recordset.Close
        query_di_interrogazione = "UPDATE Student Set IsActive = 1 WHERE IsActive = 0 AND Activation = '"& activation_code &"'"
        recordset.Open query_di_interrogazione,connessione
    %>
