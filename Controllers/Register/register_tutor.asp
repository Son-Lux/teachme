...
<%
    ...
    query_di_interrogazione = "SELECT COUNT(*) as Count FROM Tutor WHERE Email = '"& email &"'"
    ...
    query_di_interrogazione = "INSERT INTO Tutor (Name,Surname,Email,Address,Lat,Lon,Username,Password,CheckToLogIn,Activation,p_iva,cf,mobile,pageUrl,AccountType)"
    query_di_interrogazione = query_di_interrogazione + " VALUES ('"&name&"','"&surname&"','"&email&"','"&address&"','"&coord(0)&"','"&coord(1)&"','"&email&"','"&password&"','"&email&"|"&password&"','"&activation&"','"&p_iva&"','"&cf&"','"&mobile&"','"&pageUrl&"','"&account_type&"')"
    ...
    With iMsg
       ...
       .From = invioDa
       .Sender = invioDa
       .Subject = "Registrazione Tutor " & nomeDominio
       .HTMLBody = "Ciao,<br /> Benvenuto su TeachMe, clicca <a href='http://teachme.tips/Controllers/Register/activate_tutor_account.asp?c="&activation&"'>qui</a> per attivare il tuo account" 
       .Send
    End With
