...
<%
    ...
    query_di_interrogazione = "SELECT COUNT(*) As Count FROM private WHERE IsActive=0 AND Activation = '"& activation_code &"'"
    recordset.Open query_di_interrogazione,connessione

    If recordset("Count") = 1 Then
        recordset.Close
        query_di_interrogazione = "UPDATE private Set IsActive = 1 WHERE IsActive = 0 AND Activation = '"& activation_code &"'"
        recordset.Open query_di_interrogazione,connessione
    %>
