...
<%
    ...
    If request.QueryString("t") <> "" Then
        tutor_id = request.QueryString("t")
    End If
  ...
  If tutor_id <> "" Then
    ...
    query_di_interrogazione="SELECT Name, IsPrivate, pageUrl, Address, p_iva, Email, mobile, facebookpage, url, Not_avaliable_days, description, paymentmethod_cash, paymentmethod_creditcard, paymentmethod_bank, AccountType, AccountExpiringDate FROM Tutor WHERE TutorID = " & tutor_id
	recordset.Open query_di_interrogazione,connessione
    TutorName = recordset("Name")
...
<div class="main">
    <div class="container">
        ...
        <ul>
            ...
            <%Else %>
            Il tuo url personale: http://teachme.tips/Tutor_Index.asp?t=<%=tutor_id %>
            <%End If %>
            <%If url<>"" Then %>
        ...
        <div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form class="form-horizontal change_admin_form" role="form" name="change_account_form" method="post" action="../../Controllers/Account/Change_account.asp">
        ...
        <%
            If Session("tutor_logged") Then
            query_di_interrogazione="SELECT Class_LessonsStudentsID, Name FROM Class_Lessons INNER JOIN Class_LessonsStudents ON Class_LessonsStudents.Class_LessonsID = Class_Lessons.Class_LessonID WHERE IsNew = true AND TutorID = "&tutor_id
	        recordset.Open query_di_interrogazione,connessione
        ...
        <div class="class_lesson_container">
            <%
                query_di_interrogazione = "SELECT DISTINCT SchoolSubject.SchoolSubjectID, SchoolSubject.Name as SchoolSubjectName , Class.Name as ClassName, SchoolSubject.Price as Price FROM (TutorSchoolSubject INNER JOIN SchoolSubject ON SchoolSubject.SchoolSubjectID = TutorSchoolSubject.SchoolSubjectID) INNER JOIN Class ON Class.ClassID = SchoolSubject.ClassID WHERE TutorID = "&tutor_id
                recordset.Open query_di_interrogazione,connessione  
                
                If Not recordset.EOF Then
                    Do While Not recordset.eof
                        If Not Session("tutor_logged") Then
            %> <a href="tutorAvailability.asp?t=<%=tutor_id %>&ss=<%=recordset("SchoolSubjectID") %>" class="col-md-3"><span><%= recordset("SchoolSubjectName")%> (<%= recordset("ClassName")%>) - <%= recordset("Price")%>&euro; all'ora.</span></a><%
                        ElseIf Session("tutor_logged") Then
            %>
        ...
        <%If Session("tutor_logged") or Session("student_logged") Then %>
        <h2>I miei corsi:</h2>
        <br />
        <div class="class_lesson_container">
            <% If Not Session("student_logged") and Not Session("tutor_logged") Then %>
            <div class="not_logged">
                Per eseguire qualsiasi operazione col tutor devi essere registrato e loggato.
                <br />
                <br />
            ...
            <%
                ...
                If Session("student_logged") Then
                    query_di_interrogazione="SELECT Class_Lessons.Class_LessonID as Class_LessonID, Class_Lessons.Name, Class_Lessons.MenberNumber, Class_Lessons.DateStart, Class_Lessons.DateEnd, Class_Lessons.Price, Class_Lessons.Description, Class_Lessons.TakenSeats, Class_Lessons.TutorID, (SELECT COUNT(*) FROM Class_LessonsStudents WHERE StudentID = "&Session("Student_id")&" AND Class_LessonsStudents.Class_LessonsID=Class_LessonID ) AS AlreadyHere, Place FROM Class_Lessons WHERE (((Class_Lessons.[TutorID])="&tutor_id&"));"
                else
                    query_di_interrogazione="SELECT Class_LessonID, Name, MenberNumber, DateStart, DateEnd, Description, TakenSeats, TutorID, Place, Price FROM Class_Lessons WHERE TutorID = "&tutor_id
                end if
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $("#calendar").fullCalendar({
                    ...
                    events: [
                        ...
                        <% End If%>
                        <% If Session("tutor_logged") Then %>
                            //Ora segno gli impegni provvisori del tutor
                        <%
                            query_di_interrogazione="SELECT LessonID,DateStart,DateEnd, Student.Name +' '+ Student.Surname as StudentName, Mobile, Email FROM (Lessons INNER JOIN Student ON LEssons.StudentID = Student.StudentID) WHERE Accepted = 0 AND Lessons.TutorID ="&tutor_id
                recordset.Open query_di_interrogazione,connessione
                If Not recordset.EOF Then
                Do While Not recordset.eof
            %>
            { title:"provvisorio:\n<%=recordset("StudentName") %>\n <%=recordset("mobile")%> - <%=recordset("email")%>", start: "<%=recordset("DateStart") %>", end: "<%=recordset("DateEnd") %>" , color: "#d35f39" }
                        <%
                            ...
                            recordset.close
                        %>
                        //Ora segno gli impegni confermati del tutor
                        <%
                            query_di_interrogazione="SELECT LessonID,DateStart,DateEnd, Student.Name +' '+ Student.Surname as StudentName, Mobile, Email FROM (Lessons INNER JOIN Student ON LEssons.StudentID = Student.StudentID) WHERE Accepted = 1 AND Lessons.TutorID ="&tutor_id
            recordset.Open query_di_interrogazione,connessione
            If Not recordset.EOF Then
            Do While Not recordset.eof
        %>
        ...
        %>
    <% End If %>
    //Ora segno i corsi del tutor
    <%
        query_di_interrogazione="SELECT Name, DateStart, DateEnd FROM Class_Lessons WHERE TutorID = "&tutor_id
            recordset.Open query_di_interrogazione,connessione
            If Not recordset.EOF Then
            Do While Not recordset.eof
        %>
