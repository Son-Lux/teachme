...
<div class="main">
    ...
    privateID = request.querystring("t")
		...
		$("#calendar").fullCalendar({
            ...
            select: function(start, end, allDay) {
                ...
                else
                {
				    ...
				    if (title) {
					    eventData = {
                            id: "<%=StudentID %><%=privateID %><%=SchoolSubjectID %>"+moment(),
						    title: title,
						    start: start,
						    end: end,
                            editable:false
            ...
            ,events: [
                ...
                //Ora segno gli giorni in cui il private non è disponibile
                <%
                    query_di_interrogazione="SELECT Not_avaliable_days FROM private WHERE privateID ="&privateID
	                recordset.Open query_di_interrogazione,connessione
                ...
                <%
                    ...
                    recordset.close
                %>
                //Ora segno gli impegni che il private ha già preso
                <%
                    query_di_interrogazione="SELECT LessonID,DateStart,DateEnd FROM Lessons WHERE Lessons.privateID ="&privateID& " AND Lessons.StudentId <> "&StudentID
	                recordset.Open query_di_interrogazione,connessione
                    If Not recordset.EOF Then
                        Do While Not recordset.eof
                %>
